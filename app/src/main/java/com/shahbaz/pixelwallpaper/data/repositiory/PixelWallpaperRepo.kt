...
import com.shahbaz.pixelwallpaper.data.pagging.SearchWallpaperPagingSource
...
class PixelWallpaperRepo(
    ...
    fun searchWallpaper(query: String) = Pager(
        config = PagingConfig(pageSize = Constant.PER_PAGE_ITEMS, prefetchDistance = 10),
        pagingSourceFactory = { SearchWallpaperPagingSource(query = query, api = api) }
    )
}